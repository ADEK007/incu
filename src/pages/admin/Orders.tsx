...
const AdminOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const merged = ordersData.map((order: any) => ({
        ...order,
  ...
  const updateOrderStatus = async (orderId: string, status: string) => {
    try {
      const { error } = await supabase
        .from('orders')
        .update({ status: status as 'pending' | 'processing' | 'shipped' | 'delivered' | 'cancelled' })
        .eq('id', orderId);
      ...
      <div className="space-y-12">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          ...
          <div className="bg-indigo-50 px-6 py-3 rounded-2xl">
            <span className="text-indigo-600 font-black text-lg">{orders.length}</span>
            <span className="text-indigo-400 font-bold text-sm ml-2">Total Orders</span>
          </div>
        </div>
